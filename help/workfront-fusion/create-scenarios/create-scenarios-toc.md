---
content-type: reference
title: 'Criar cenários: índice do artigo'
description: Esta seção contém artigos relacionados à criação de cenários no Adobe Workfront Fusion.
author: Becky
feature: Workfront Fusion
recommendations: noDisplay, noCatalog
exl-id: 433e0730-1955-4582-984a-94b98e8366b5
TQID: https://experienceleague.adobe.com/mdEuXi2Pwa4ARFyL9oWRU7S-SpchAbJV6Li6Lg-pajA
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
source-git-commit: 219b9dbf3a7e4be1676b21bc3d3752d70d743b13
workflow-type: tm+mt
source-wordcount: 33
ht-degree: 39%

---

# Criar cenários: índice do artigo

* [Adicionar módulos](/help/workfront-fusion/create-scenarios/add-modules/add-modules-toc.md)
* [Mapear dados](/help/workfront-fusion/create-scenarios/map-data/map-data-toc.md)
* [Conectar a aplicativos](/help/workfront-fusion/create-scenarios/connect-to-apps/connect-to-apps-toc.md)
* [Configurar tratamento de erros](/help/workfront-fusion/create-scenarios/config-error-handling/config-error-handling-toc.md)
* [Definir configurações do cenário](/help/workfront-fusion/create-scenarios/config-scenarios-settings/config-scenario-settings-toc.md)
